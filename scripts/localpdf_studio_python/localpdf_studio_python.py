...
class redact_pdf:
    ...
    def main():
        ...
        redaction_group.add_argument('--redactions')
        redaction_group.add_argument('--redactions-file')
        parser.add_argument('--json', action='store_true')
        args = parser.parse_args()
        ...
        try:
            ...
            print(json.dumps({"success": False, "error": f"Failed to parse redactions: {str(e)}"}))
            return 1

        for i, redact in enumerate(redactions_data):
            for field in ['page', 'x', 'y', 'width', 'height', 'color']:
                if field not in redact:
                    print(json.dumps({"success": False, "error": f"Redaction {i} missing required field: {field}"}))
                    return 1

        return _redact_apply(args.input_pdf, args.output_pdf, redactions_data)
